...
public static class MomCareSeedData
{
    ...
    public static async Task SeedAsync(IServiceProvider services)
    {
        ...
        var babyBath = await EnsureServiceAsync(context, "Dịch vụ tắm bé & vệ sinh sơ sinh tại nhà", "Điều dưỡng hướng dẫn kỹ thuật tắm bé an toàn — tắm nước ấm, vệ sinh rốn, mắt, tai và chăm sóc da cho trẻ sơ sinh ngay tại nhà.", "cham-be-so-sinh", 350_000m, 45, "active", "single");
        var motherHealth = await EnsureServiceAsync(context, "Theo dõi phục hồi sức khỏe mẹ sau sinh", "Kiểm tra các dấu hiệu hồi phục sau sinh: vết mổ, sản dịch, co hồi tử cung, huyết áp và hỗ trợ mẹ nhận biết sớm dấu hiệu bất thường.", "cham-me-sau-sinh", 500_000m, 60, "active", "single");
        var babyHealth = await EnsureServiceAsync(context, "Theo dõi phát triển thể chất & phản xạ sơ sinh", "Đo cân nặng, chiều dài, vòng đầu, kiểm tra phản xạ nguyên thủy và các mốc phát triển quan trọng của trẻ trong giai đoạn sơ sinh.", "cham-be-so-sinh", 450_000m, 60, "active", "single");
        var lactation = await EnsureServiceAsync(context, "Tư vấn & hỗ trợ cho bú và xử lý tắc tia sữa", "Hỗ trợ mẹ xử lý tắc tia sữa, đau đầu ti và các vấn đề cho con bú; hướng dẫn tư thế bú đúng, lịch bú khoa học, kích thích nguồn sữa.", "tu-van-tai-nha", 600_000m, 90, "active", "single");
        var massage = await EnsureServiceAsync(context, "Massage phục hồi cơ thể & giảm đau nhức sau sinh", "Liệu trình massage chuyên sâu — giảm đau vùng lưng, vai gáy, kích thích tuần hoàn máu, hỗ trợ phục hồi cơ xương khớp sau sinh.", "phuc-hoi-suc-khoe", 700_000m, 90, "active", "single");
        var nutrition = await EnsureServiceAsync(context, "Tư vấn dinh dưỡng & xây dựng thực đơn sau sinh", "Cá nhân hóa thực đơn giàu dưỡng chất cho mẹ sau sinh — hỗ trợ phục hồi sức khỏe, lợi sữa và cân bằng dinh dưỡng cho con bú.", "tu-van-tai-nha", 550_000m, 60, "active", "single");
        var nightCare = await EnsureServiceAsync(context, "Dịch vụ chăm bé ban đêm giúp mẹ ngủ trọn giấc", "Điều dưỡng ở lại xuyên đêm chăm sóc bé — cho bú, thay bỉm, vỗ ợ, dỗ bé ngủ — để mẹ có giấc ngủ liên hoàn phục hồi sức khỏe.", "cham-be-so-sinh", 1_200_000m, 480, "active", "single");
        var houseSupport = await EnsureServiceAsync(context, "Hỗ trợ việc nhà & dọn dẹp không gian chăm sóc", "Giúp gia đình dọn dẹp, nấu ăn, giặt giũ và sắp xếp không gian sống sạch sẽ để mẹ và bé luôn ở trong môi trường thoải mái, an toàn.", "ho-tro-gia-dinh", 400_000m, 120, "active", "single");
        var mentalWellness = await EnsureServiceAsync(context, "Tham vấn tâm lý & hỗ trợ tinh thần sau sinh", "Đồng hành cùng mẹ trong giai đoạn nhạy cảm — lắng nghe, chia sẻ và hướng dẫn kỹ thuật thư giãn giúp giảm lo âu, căng thẳng, baby blues.", "ho-tro-tinh-than", 500_000m, 60, "active", "single");
        var emergencyConsultation = await EnsureServiceAsync(context, "Tư vấn khẩn cấp & xử trí tình huống tại nhà", "Điều dưỡng đến tận nơi hoặc tư vấn trực tuyến khi mẹ gặp tình huống cần xử trí y tế khẩn — sốt cao, chảy máu bất thường, khó thở.", "tu-van-tai-nha", 350_000m, 30, "active", "single");
        var miniConsultation = await EnsureServiceAsync(context, "Tư vấn nhanh qua video & điện thoại sau sinh", "Cuộc gọi tư vấn ngắn 15-20 phút với điều dưỡng để gia đình hỏi nhanh cách xử trí các vấn đề thường gặp: tắc sữa, bé quấy khóc, rốn rỉ dịch.", "tu-van-tai-nha", 200_000m, 15, "active", "single");
        ...
        var pkgGiamNhucMoi = await EnsureServiceAsync(context, "Gói Massage Giảm Nhức Mỏi & Phục Hồi Cơ Thể Sau Sinh", "Gói 6 buổi massage trị liệu — tập trung giảm đau vùng lưng, vai gáy, hông, kết hợp kỹ thuật bấm huyệt giúp mẹ phục hồi thể lực và thư giãn toàn thân.", "goi-dich-vu", 3_600_000m, 60, "active", "package", 6, "postpartum-massage,mother-health-monitoring", GeneratePackageScheduleJson(6, "postpartum-massage,mother-health-monitoring"));
        ...
        var pkgPhucHoi = await EnsureServiceAsync(context, "Gói Phục Hồi Sức Khỏe & Tinh Thần Sau Sinh Toàn Diện", "Gói 12 buổi toàn diện — massage phục hồi, tư vấn dinh dưỡng cá nhân hóa, theo dõi sức khỏe mẹ và tham vấn tâm lý giúp mẹ vượt qua giai đoạn hậu sản nhẹ nhàng.", "goi-dich-vu", 7_200_000m, 90, "active", "package", 12, "postpartum-massage,nutrition-consultation,mother-health-monitoring", GeneratePackageScheduleJson(12, "postpartum-massage,nutrition-consultation,mother-health-monitoring"));
        ...
        var pkgChuyenSauBung = await EnsureServiceAsync(context, "Gói Phục Hồi Vóc Dáng & Định Hình Cơ Thể Sau Sinh", "Gói 20 buổi massage định hình chuyên sâu — tập trung phục hồi vùng bụng, eo, hông sau sinh, giúp mẹ lấy lại vóc dáng với lộ trình khoa học và an toàn.", "goi-dich-vu", 14_000_000m, 90, "active", "package", 20, "postpartum-massage,mother-health-monitoring", GeneratePackageScheduleJson(20, "postpartum-massage,mother-health-monitoring"));
